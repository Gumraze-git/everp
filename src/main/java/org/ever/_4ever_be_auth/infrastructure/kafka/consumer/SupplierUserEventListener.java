...
public class SupplierUserEventListener {
    ...
    private final SupplierUserSagaPort supplierUserSagaPort;
    ...
    public void handleEvent(CreateSupplierUserEvent event) {
        log.info(\"[KAFKA][SUPPLIER] CreateSupplierUserEvent 수신 - txId: {}, userId: {}\", event.getTransactionId(), event.getUserId());
        ...
        sagaTransactionManager.executeSagaWithId(transactionId, () -> {
            try {
                CreateAuthUserResultEvent result = supplierUserSagaPort.handleCreateSupplierUser(event);
                sagaTransactionStatusService.markCompleted(transactionId);
                kafkaProducerService.sendEventSync(
                        KafkaTopicConfig.SUPPLIER_USER_RESULT_TOPIC,
                        result.getUserId(),
                        result
                );
                log.info(\"[KAFKA][SUPPLIER] 공급사 사용자 생성 완료 이벤트 발행 - txId: {}\", transactionId);
            } catch (Exception error) {
                log.error(\"[KAFKA][SUPPLIER] 공급사 사용자 생성 처리 실패 - txId: {}, cause: {}\", transactionId, error.getMessage(), error);

...
export const getDashboardStats = async (): Promise<DashboardStatRes> => {
  const res = await axios.get<DashboardStatRes>(DASHBOARD_ENDPOINTS.STATS);
  return res.data;
};

// ----------------------- 워크플로우 현황 -----------------------
export const getWorkflowStatus = async (): Promise<DashboardWorkflowRes> => {
  const res = await axios.get<DashboardWorkflowRes>(DASHBOARD_ENDPOINTS.WORKFLOW_STATUS);
  return res.data;
};

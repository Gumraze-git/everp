...
public class PpHttpServiceImpl implements PpHttpService {
    ...
    @Override
    public ResponseEntity<ApiResponse<List<DashboardWorkflowItemDto>>> getDashboardQuotationsToProduction(
            String userId,
            Integer size
    ) {
        ...
        if (userId == null || userId.isBlank()) {
            return ResponseEntity.badRequest().body(
                    ApiResponse.fail("production userId is required", HttpStatus.BAD_REQUEST, null)
            );
        }
        ...
        try {
            ...
            ApiResponse<List<DashboardWorkflowItemDto>> body = scmClient.get()
                    .uri(uriBuilder -> uriBuilder
                            .path("/scm-pp/dashboard/quotations/production")
            ...
            if (body == null) {
                log.error("[ERROR][DASHBOARD][PP] SCM 서버 응답이 null");
            ...
            if (data == null) {
                log.error("[ERROR][DASHBOARD][PP] SCM 서버 응답에 data 필드가 없음");
            ...
            return ResponseEntity.ok(ApiResponse.success(data, "생산 전환 견적 목록 조회 성공", HttpStatus.OK));
        } catch (WebClientResponseException ex) {
            return handleWebClientError("대시보드 생산 전환 견적 목록 조회", ex);
        } catch (Exception e) {
            log.error("[ERROR][DASHBOARD][PP] 생산 전환 견적 목록 조회 중 에러 발생", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.fail("대시보드 생산 전환 견적 목록 조회 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR, null)
            );
        }
    }

    @Override
    public ResponseEntity<ApiResponse<List<DashboardWorkflowItemDto>>> getDashboardProductionInProgress(String userId, Integer size) {
        ...
        try {
            ...
            ApiResponse<List<DashboardWorkflowItemDto>> body = scmClient.get()
                    .uri(uriBuilder -> uriBuilder
                            .path("/scm-pp/dashboard/production/in-progress")
                            .queryParam("userId", userId)
                            .queryParam("size", pageSize)
                            .build())
                    .retrieve()
            ...
            return ResponseEntity.ok(ApiResponse.success(data, "생산 진행 목록 조회 성공", HttpStatus.OK));
        } catch (WebClientResponseException ex) {
            return handleWebClientError("대시보드 생산 진행 목록 조회", ex);
        } catch (Exception e) {
            log.error("[ERROR][DASHBOARD][PP] 생산 진행 목록 조회 중 에러 발생", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.fail("대시보드 생산 진행 목록 조회 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR, null)
            );
        }
    }

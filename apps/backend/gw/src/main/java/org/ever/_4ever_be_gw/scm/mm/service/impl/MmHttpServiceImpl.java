...
public class MmHttpServiceImpl implements MmHttpService {
    ...
    public ResponseEntity<ApiResponse<List<DashboardWorkflowItemDto>>> getDashboardCompanyPurchaseOrderList(String userId, int size) {
        log.debug("[DASHBOARD][MM] 기업 발주서 목록 요청 - size: {}", size);
    ...
    @Override
    public ResponseEntity<ApiResponse<List<DashboardWorkflowItemDto>>> getDashboardPurchaseOrdersOverall(int size) {
        log.debug("[DASHBOARD][MM] 전체 발주서(PO) 목록 요청 - size: {}", size);
        ...
        try {
            ...
            log.info("[INFO][DASHBOARD][MM] 전체 발주서 목록 조회 성공");
            return ResponseEntity.ok(ApiResponse.success(data, "전체 발주서 목록 조회 성공", HttpStatus.OK));
        } catch (WebClientResponseException ex) {
            return handleWebClientError("대시보드 전체 발주서 목록 조회", ex);
        } catch (Exception e) {
            log.error("[ERROR][DASHBOARD][MM] 전체 발주서 목록 조회 중 에러 발생", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.fail("대시보드 전체 발주서 목록 조회 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR, null)
            );
        }
    }

    @Override
    public ResponseEntity<ApiResponse<List<DashboardWorkflowItemDto>>> getDashboardPurchaseRequestsOverall(int size) {
        log.debug("[DASHBOARD][MM] 전체 구매 요청 목록 요청 - size: {}", size);
        ...
        try {
            ...
            log.info("[INFO][DASHBOARD][MM] 전체 구매 요청 목록 조회 성공");
            return ResponseEntity.ok(ApiResponse.success(data, "전체 구매 요청 목록 조회 성공", HttpStatus.OK));
        } catch (WebClientResponseException ex) {
            return handleWebClientError("대시보드 전체 구매 요청 목록 조회", ex);
        } catch (Exception e) {
            log.error("[ERROR][DASHBOARD][MM] 전체 구매 요청 목록 조회 중 에러 발생", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResponse.fail("대시보드 전체 구매 요청 목록 조회 중 오류가 발생했습니다.", HttpStatus.INTERNAL_SERVER_ERROR, null)
            );
        }
    }

...
public class BusinessQuotationServiceAdapter implements BusinessQuotationServicePort {
    ...
    public BusinessQuotationListResponseDto getQuotationList(
        ...
        try {
            ...
            BusinessQuotationListResponseDto response = restClient.get()
                    .uri(uri)
                    .retrieve()
                    .body(BusinessQuotationListResponseDto.class);

            if (response != null) {
                log.info("Business 서비스 호출 성공 - 견적 목록 조회: 총 {}건", response.getTotal());
                return response;
            }

            log.error("Business 서비스 응답 실패 - response is null");
    ...
    public BusinessQuotationDto getQuotationById(String quotationId) {
        ...
        try {
            ...
            BusinessQuotationDto response = restClient.get()
                    .uri(uri)
                    .retrieve()
                    .body(BusinessQuotationDto.class);

...
import org.ever._4ever_be_alarm.common.audit.EntityAuditListener;
import org.ever._4ever_be_alarm.common.audit.TransactionEntityChangeCollector;
...
public class SagaTransactionManager {
    ...
    public <T> T executeSagaWithId(String externalTransactionId, Supplier<T> action) {
        String transactionId =
            (externalTransactionId != null) ? externalTransactionId : UUID.randomUUID().toString();
        try {
            ...
            log.error("Saga transaction {} failed, initiating compensation: {}", transactionId,
                e.getMessage());
            // 롤백 수행
            try {
                compensationService.compensate(transactionId);
            } catch (Exception ex) {
                log.error("Compensation for transaction {} failed: {}", transactionId,
                    ex.getMessage());
            }
    ...
    public boolean compensateExternalTransaction(String transactionId) {
        ...
        try {
            ...
            log.error("Failed to compensate external transaction {}: {}", transactionId,
                e.getMessage(), e);
            return false;
        }
    }

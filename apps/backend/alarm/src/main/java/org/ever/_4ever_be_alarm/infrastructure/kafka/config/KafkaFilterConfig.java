package org.ever._4ever_be_alarm.infrastructure.kafka.config;
...
public class KafkaFilterConfig {
    ...
    public RecordFilterStrategy<String, Object> paymentFilter() {
        return new RecordFilterStrategy<String, Object>() {
            ...
            public boolean filter(ConsumerRecord<String, Object> record) {
                ...
                // 예시 3: 비즈니스 로직 기반 필터링
                // AlarmEvent event = (AlarmEvent) record.value();
     ...
     */
    // @Bean
    // public RecordFilterStrategy<String, Object> highValueAlarmFilter() {
    //     return record -> {
    //         // 고액 결제만 처리하는 필터
    //         if (record.value() instanceof AlarmEvent) {
    //             AlarmEvent event = (AlarmEvent) record.value();
